...
class DeltaHeatmapItem(FinPlotItem):
    ...
    def generate_picture(self, boundingRect):
        ...
        vup = np.nanpercentile(values[values > 0], 90)
        vlow = np.nanpercentile(values[values < 0], 10)
        ...
        vbull = rows[rows[:,2] > vup]
        vbear = rows[rows[:,2] < vlow]

        # np.empty((len(values[values > vup]), ))

        # if np.nanmin(values) > 0:
        #     values = (values / vlow * (values < 0) + values / vup * (values > 0)) / (1 + self.whiteout)# overshoot for coloring
        # else:
...
class ScatterLabelItem(FinPlotItem):
    def __init__(self, ax, datasrc, color, anchor, qfont=None):
        self.color = color
        self.text_items = {}
        self.anchor = anchor
        self.show = False
...
def create_plot(title='Finance Plot', rows=1, init_zoom_periods=1e10, maximize=True, yscale='linear', row_stretch_factors=None):
...
def delta_heatmap(datasrc, ax=None, **kwargs):
    ...
    _set_datasrc(ax, datasrc)
    item = DeltaHeatmapItem(ax=ax, datasrc=datasrc, **kwargs)
    item.update_data = partial(_update_data, None, None, item)
    item.update_gfx = partial(_update_gfx, item)
    item.setZValue(-30)
    ax.addItem(item)
...
def plot(x, y=None, color=None, width=1, ax=None, style=None, legend=None, zoomscale=True, **kwargs):
    ...
    if style is None or any(ch in style for ch in '-_.'):
        ...
        item = ax.plot(x, y, pen=_makepen(color=used_color, style=style, width=width), name=legend, connect=connect_dots, **kwargs)
...
def labels(x, y=None, labels=None, color=None, ax=None, anchor=(0.5,1), **kwargs):
    ax = _create_plot(ax=ax, maximize=False)
...
def add_line(p0, p1, color=draw_line_color, width=1, style=None, interactive=False, ax=None):
    ...
    x_pts = _pdtime2index(ax, pd.Series([p0[0], p1[0]]), any_end=True)
    ix = ax.vb.yscale.invxform
